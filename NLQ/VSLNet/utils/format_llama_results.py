...
def load_clips_dict(clips_dict_path):
    """Load videos metadata dictionary from JSON."""
    with open(clips_dict_path, "r") as f:
        return {entry["clip_uid"]: entry for entry in json.load(f)}
...
def format_llama_results_for_vslnet(llama_results, output_dir, split_ratios=(0.7, 0.15, 0.15)):
    ...
    clips_dict = load_clips_dict("/home/alexhax/aml/ego4d_data/clips_dict.json")
    ...
    for split, video_list in splits.items():
        for video_uid in video_list:
            ...
            for narration_entry in llama_results[video_uid]:
                query_start_sec = narration_entry["start_sec"]
                query_end_sec = narration_entry["end_sec"]
